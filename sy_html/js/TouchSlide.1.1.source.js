var TouchSlide = function(a) {
	a = a || {};
	var opts = {
		slideCell: a.slideCell || "#touchSlide", //运行效果主对象，必须用id！
	...
	var pageState = obj(opts.pageStateCell, slideCell)[0];
	...
	var showMsgUl = '';
	/*if(document.getElementById("parkingdetails").style.display=='none'){ //地图信息窗体
		showMsgUl = document.body.clientWidth - 22;
	}else{ //停车场详情*/
		showMsgUl = document.body.clientWidth;
	//}
	...
	conBox.style.cssText = "width:" + tempSize * slideW + "px;" + "overflow:hidden;padding:0;margin:0;";
	for (var i = 0; i < tempSize; i++) {
		conBox.children[i].style.cssText = "width:" + showMsgUl + "px";
	}
	...
	var orientationChange = function() {
		slideW = twCell.clientWidth + 20;  //宽度+20
	...
	//效果函数
	var doPlay = function(isTouch, indexClick) {
	...
	var tEnd = function(e) {
		...
		if (a.slideCell === '#leftTabBox') {				
			indexClick = index;
			if($(markerImg[indexClick]).attr("class").indexOf("bg_blue") < 1){
			...
			for (var k = 0; k < markerImg.length; k++) {
				var emptyBed = '';
				...
				if(emptyParkPlaceCount == 0 || emptyParkPlaceCount < -1){
					...
					var emptyBed = emptyParkPlaceCount;
				}else{
					var emptyBed = emptyParkPlaceCount;
				}
				var beds = aroundParking[k].attributes.parkPlaceCount;
				if (k == indexClick) {
					continue;
				}
				if (emptyParkPlaceCount / beds > 0.5) {
					$(markerImg[k]).addClass("bg_green");
				} else if (emptyParkPlaceCount >= 10) {
					$(markerImg[k]).addClass("bg_orange");
				} else if (emptyParkPlaceCount > 0) {
					$(markerImg[k]).addClass("bg_red");
				} else {
					$(markerImg[k]).addClass("bg_gray");
				}
	
			}				
			parkClicked = aroundParking[index].attributes;
			tname = aroundParking[index].attributes.name;
			parkId = aroundParking[index].attributes.id;
