...
function addmarker(i, d) {
	...
	markerTitil[i] = "¥" + aroundParking[i].attributes.park_qh||"";
	...
	var markerOption = {
		...
		title: markerContent //自定义点标记覆盖物内容
	};
	mar[i] = new BMap.Marker(new BMap.Point(lngX, latY));
//	marker.push(new BMap.LngLat(lngX, latY));
	...
	//for (var x = 0; x < markerobj.length; x++) {
		 	//		mapObj.clearOverlays(markerobj[x]); //批量删除以前的停车场标注
		 	//	}
	//mapObj.clearOverlays(markerobj[i]); //批量删除以前的停车场标注
	var label = new BMap.Label(markerTitil[i],{offset:new BMap.Pixel(-18, -36)});
	label.setStyle({
			 color : "#222222",
			 fontSize : "12px",
			 height : "20px",
	...
	mapObj.addOverlay(markerobj[i]);
	markerobj[i].setLabel(label);
...
// 			var driving = new BMap.DrivingRoute(mapObj, {renderOptions:{
// 				map: mapObj, autoViewport: true},
// 				policy: route});
// 			driving.search(start,end);
// 		}

   // var MDrive = new BMap.DrivingRoute(mapObj, {renderOptions:{map: mapObj, autoViewport: true}});
	// var MDrive;
	// new BMap.DrivingRoute(mapObj, function() {
	// 	var DrivingOption = {
    ...
    // var options = {
        // onSearchComplete: function(results){
            ...
            // if (driving.getStatus() == BMAP_STATUS_SUCCESS){
	         ...
	         //        route_text = "<table cellspacing=\"5px\"><tr><td><img src=\"./image/start.gif\" />&nbsp;&nbsp;当前位置</td><td id='btn_close' class='icon_close'>退出导航</td></tr>" + s.join("<br/>") + "<tr><td><img src=\"./image/end.gif\" />&nbsp;&nbsp;" + tname + "</td></tr></table>";
      ...
      //   }
    // };
    // var driving = new BMap.DrivingRoute(mapObj, options);
   
    // driving.search(start_xy, end_xy);
	
    var driving = new BMap.DrivingRoute(mapObj, {renderOptions: {map: mapObj, panel: "result", autoViewport: true}});
    flagGPS = true;
    driving.search(start_xy, end_xy)
	$("#tip").append("<div id='btn_close' class='icon_close'>退出导航</div>");
...
function drivingDrawLine(s) {
	...
	if (endmarker != null) {
		mapObj.clearOverlays(endmarker);
	}
	if (polyline != null) {
		mapObj.clearOverlays(polyline);
	}
	...
	extra_path1.push(step[0].path[0]);
	...
	var path_xy = step[(step.length - 1)].path;
	...
	for (var s = 0; s < step.length; s++) {
		var plength = step[s].path.length;
		for (var p = 0; p < plength; p++) {
			drawpath.push(step[s].path[p]);
		}
	}
	...
	if($(this).hasClass("tip_standard")){  //停车费，切换至剩余车位
		for (var k in aroundParking) {
			...
			if(emptyParkPlaceCount == 0 || emptyParkPlaceCount < -1){
				...
				markerTitil[k] = parkPlaceCount;
			}else{
				$(markerSpan[k]).html(emptyParkPlaceCount);
				markerTitil[k] = emptyParkPlaceCount;
			}
		}
		...
		for (var k in aroundParking) {
			...
			markerTitil[k] = "¥" + aroundParking[k].attributes.park_qh;
		}
